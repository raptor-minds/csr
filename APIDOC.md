...
**接口路径**: `GET /api/user-activity/latest`
...
**请求示例**:
```
GET /api/user-activity/latest?userId=1&activityId=2
...
| activityId | int   | 活动ID              |
| state      | string| 用户活动状态         |
| endorsedBy | int   | 审核人ID            |
| endorsedAt | string| 审核时间（ISO格式）  |
| createdAt  | string| 创建时间（ISO格式）  |
| chainId    | string| 区块链ID            |
| detail     | object| 活动详情（结构依模板）|